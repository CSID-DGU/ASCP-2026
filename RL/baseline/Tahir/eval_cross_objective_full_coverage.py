...
배경(log/0712/tahir_baseline_비교방법_쉬운설명.md, log/0711/paper/07_Tahir_RL보상수정_실험.md):
  ...
  우리 쪽은 부분집합 제한 없이 원래 evaluate_ip.py 파이프라인을 3,500편 전체에
  그대로 돌린다(원래도 100% 커버가 구조적으로 보장됨). 기존 코드
  (eval_same_subset.py, evaluate_ip.py, Tahir/solver/*)는 전혀 수정하지 않고, 이
...
def main():
    ...
    print()
    print("=" * 70)
    print("2단계: Tahir 쪽 — 커버 못 한 flight을 1-leg pairing으로 강제 배정(패딩)")
    print("=" * 70)
    ...
    print(f"  Tahir가 이미 커버: {len(covered_fids)}개 (coverage {tahir_result['coverage']*100:.1f}%)")
    ...
    tahir_full_cost = tahir_result["mip_obj"] + padding_cost
    tahir_full_n_pairings = len(tahir_result["selected_pairings"]) + len(uncovered_fids)

    print(f"  Tahir 원래 비용(커버 {tahir_result['coverage']*100:.1f}%만): {tahir_result['mip_obj']:>14.2f}")
    print(f"  패딩 비용(미커버 {len(uncovered_fids)}개, 1-leg 강제):        {padding_cost:>14.2f}")
    print(f"  = Tahir 비용(100% 커버로 패딩):                        {tahir_full_cost:>14.2f}")
    print(f"  Tahir pairing 수(100% 커버로 패딩): {tahir_result['n_pairings'] if 'n_pairings' in tahir_result else len(tahir_result['selected_pairings'])} + {len(uncovered_fids)} = {tahir_full_n_pairings}")
    ...
    print()
    print("=" * 70)
    print("최종 — 양쪽 다 100% 커버 기준, Tahir 자신의 목적함수(mip_obj/pairing_cost)")
    print("=" * 70)
    ...
    print(f"  → 둘 다 100% 커버 기준, Tahir 자신의 objective로 채점해도 우리가 "
    ...
    print()
    print("=" * 70)
    print("버전 3 — 그냥 각각 그 자체로(강제/패딩 없이, 각자 native 지표로 비교)")
    print("=" * 70)
    ...
    print(f"  {'-'*28} {'-'*20} {'-'*20}")
    print(f"  {'coverage':<28} {tahir_result['coverage']*100:>19.1f}% {'100.0':>19}%")
    print(f"  {'n_pairings':<28} {len(tahir_result['selected_pairings']):>20d} {our_result['n_pairings']:>20d}")
