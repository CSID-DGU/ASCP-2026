...
직접 비교:
  ...
  3. 그 부분집합만으로 우리 RL+IP 파이프라인(evaluate_ip.py의 함수들을 그대로 import해서
     재사용 — 로직 수정 없음)을 실행.
  4. 양쪽 다 이 부분집합에서 coverage 100%인 상태로 dead time/FTC/deadhead/ManDays 비교.

기존 evaluate_ip.py / eval_vs_baseline.py / Tahir/eval_delta.py는 전혀 수정하지 않았고
...
import evaluate_ip                       # 재사용: 모델 로드/pool 수집/IP 함수 전부 그대로 사용
...
# ── 3. 같은 부분집합으로 우리 RL+IP 실행(evaluate_ip.py 함수 재사용) ──────────────

def run_ours_on_subset(restricted_flights, checkpoint_path, lambda_dh, ip_time_limit):
    airport_map = evaluate_ip.build_airport_map(evaluate_ip.config.AIRLINE_DATA["delta"])
