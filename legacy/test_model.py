"""Tests for the Thursday (04/02) completion criteria:
- FiLM: changing the constraint on a dummy input changes the output
- Transformer: flight vector input/output shapes are correct
- Pointer: probs output from a dummy query sums to 1
"""
...
def test_encoder():
    ...
    # Test 2: FiLM -- output changes when constraint changes
    ...
    assert diff > 0, \
        f"[FAIL] constraint 8h vs 14h outputs are identical (diff={diff})"
    print(f"[PASS] constraint 8h vs 14h output diff: {diff:.4f}")

    # Test 3: cosine similarity (for reference only, no assert)
    cos_sim = torch.nn.functional.cosine_similarity(
        enc_strict.flatten().unsqueeze(0),
        enc_relaxed.flatten().unsqueeze(0),
    ).item()
    print(f"[INFO] cosine similarity: {cos_sim:.4f} (may be high before training; target: < 0.9)")
...
def test_decoder(encoder, encoded):
    ...
    # mask: only the first 5 valid + END_PAIRING
    ...
    # Test 3: invalid flight prob = 0
    invalid_probs = probs[5:N_FLIGHTS]  # flight 5-9 are masked
    ...
    print()
    print(f"All probs: {[round(p, 4) for p in probs.detach().tolist()]}")
